...
class SimpleBenchmark {
  ...
  async add(name: string, fn: () => Promise<void>, options = {
    minSamples: 5,
    minTime: 1000, // 1 second minimum
    maxTime: 5000, // 5 seconds maximum
    ...
    const rme = (stdDev / mean) * 100; // Relative margin of error as percentage
    const hz = 1000 / mean; // Operations per second
...
async function runBenchmarks() {
  ...
  const cycleQueue = new AsyncQueue<number>(100);
